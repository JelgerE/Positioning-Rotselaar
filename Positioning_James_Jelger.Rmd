...
# 1 day
time_start <- as.POSIXct("03-05-2023", format="%d-%m-%Y", tz="UTC")
time_stop <- as.POSIXct("04-05-2023", format="%d-%m-%Y", tz="UTC")
...
int_drft <- init_drift(sync_model, detlst)
int_drft
...
sync_fit2 <- fit_offsets(clocksync = sync_fit, detlst = detlst)

plot(detlst)
latency <- SyncTagLatency(detlst)
plot(SyncTagLatency(detlst = detlst))

#detlst_tmp_2 <- synchronize(clocksync = sync_fit_2, detlst = moerrumsaan_detlst_raw)
```
